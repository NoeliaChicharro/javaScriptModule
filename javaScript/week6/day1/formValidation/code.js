...
function validateForm(){
  ...
  data.message = document.querySelector('#message');
  ...
  if(!data.email.value){
    ...
    // Otherwise, the email is not empty BUT we still should test if it is an email address
  ...
  if(data.message.textLength < 20){
    console.error('Message is too short');
    validationErrors.message = 'Your Message is too short';
  }else{
    console.info('Message correct');
  }
...
function displayErrors (errors, data) {
  ...
  if(errors.message){
    createAfter(data.message, errors.message)
  }
}
...
document.querySelector('form textarea').addEventListener('input', e => {
  ...
  // check if in the element is an object with the class .counterBox
  if (!document.querySelector('.counterBox')){
    // if there is no element with such class, we create one
    ...
    document.querySelector('.counterBox').style.color = '#b4190b';
  } else {
    // if the element does already exists we just change the value
    document.querySelector('.counterBox').innerHTML = e.target.textLength;

    // let the the counter be green if there are less then 20 inputs
