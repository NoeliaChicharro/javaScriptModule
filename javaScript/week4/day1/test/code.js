...
const person = {
  ...
  books: [
    {author: 'J.K Rolling', name: 'Harry Potter'},
    {author: 'Stephan King', name: 'Pet Sematary'},
    {author: 'Herman Melville', name: 'Moby Dick'}
  ],
  social: function() {
    console.log('My favorite book is ' + this.books[1].name + ' by ' + this.books[2].author);
  }
};
...
function deleteOr(str){
  let small = str.toLowerCase();
  if (small.indexOf('o') > -1){
      small = small.replace(/o/g, '');
  }
  return small;
}
console.log(deleteOr(string));
...
//Code
function Numberus(num) {
  if (num % 2 === 0){
    console.log('\x1b[32mYour number ' + num + ' is even');
  } else {
    console.log('\x1b[31mYour number ' + num + ' is odd');
  }
}
...
function prime(n){
 for (let i = 2; i < n; i++){
   if (n % i === 0){
     console.log('\u001b[35mYour number is not a prime');
   }
 }
  console.log('\u001b[35mYour number is a prime number');
}
...
// Code
console.log(person.books[0].author + person.books[0].name + '\n' +
  person.books[1].author + person.books[1].name + '\n' +
  person.books[2].author + person.books[2].name);

console.log('\t');

person.books.push({author: 'Charles Dickens', name: 'Great Expectations'});

person.hobby.pop();
person.hobby.shift();

person.hobby.push('Book Club');
person.hobby.push('Writing');
...
// Code
person.punctuation = function(sentence){
  sentence = sentence.replace(/!/g, '');
  return sentence;
};

console.log(person.punctuation('Hello! Yo,u terri!ble Writer!'));
